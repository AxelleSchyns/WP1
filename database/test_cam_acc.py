...
def display_cm(weight, measure, ground_truth, data, predictions, predictions_maj):
    ...
    plt.savefig(fold_path + '/cam_confusion_matrix_top1_'+measure+ '.png')
    ...
    #plt.show()
    # save the confusion matrix
    plt.savefig(fold_path + '/cam_confusion_matrix_maj_'+measure+'.png')
...
def display_precision_recall(weight, measure, ground_truth, predictions):
    ...
    fold_path = weight[0:weight.rfind("/")]
    plt.savefig(fold_path + '/cam_prec_recall_curve_'+measure+'.png')
